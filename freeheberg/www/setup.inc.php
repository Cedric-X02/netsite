 ...
 * LICENSE:
 *
 *   Copyright 2008-2019 Alessandro Briosi
 *
 ...
 * @copyright 2019 Alessandro Briosi
...
$DB_STRUCTURE = array (
  ...
  'users' => array (
     'fields' => array (
        ...
        'password' => array ( 'type' => 'char', 'size' => 250, 'extra' => '', 'null' => 'NOT NULL', ),
        'name' => array ( 'type' => 'char', 'size' => 200, 'extra' => '', 'null' => 'NOT NULL', ),
...
function setupcheck($step) {
  ...
  if (isset($_POST['check'])) {
    ...
    if (function_exists('mysqli_connect')) {
      msg('MYSQLi Support: exsists','ok');
    } else {
      msg('MYSQLi Support: not found','fail');
      msg('MYSQLi is needed if you plan to use mysql');
    }
    if (defined('PDO::ATTR_DRIVER_NAME')) {
      msg('PDO Support: exsists','ok');
      if (in_array('mysql',PDO::getAvailableDrivers())) {
        msg('PDO::mysql Support: exsists','ok');
      } else {
        msg('PDO::mysql not found! Is needed if you plan to use mysql PDO (better)','fail');
      }
    } else {
      msg('PDO Support: not found','fail');
      msg('PDO::MySQL is needed if you plan to use mysql PDO (better)');
    }
  ...
  if (isset($_POST['dbhost'])) {
    ...
    if (isset($_POST['test'])) {
      if ($_SESSION['options']['newdb']==1) {
        ...
        if ($CONFIG['database']['type']=='mysql' or $CONFIG['database']['type']=='pdo_mysql')
          $dbn = 'mysql';
        else 
          $dbn = 'postgres';
      } else {
...
function options($step) {
  ...
  if (isset($_POST['translator'])) {
    ...
    $CONFIG['site']['email']= '';
    $CONFIG['site']['footer']='<a href="http://openupload.sf.net">Open Upload</a> - Created by Alessandro Briosi &copy; 2019';
...
global $CONFIG;
    ...
    switch ($CONFIG['database']['type']) {
      case 'mysql':
        if (!($dbhandle = mysqli_connect($host,$user,$pwd,$db))) {
          if ($debug) echo '<div id="message">Reason: '.mysqli_connect_error().'</div>';
          return false;
        }
        break;
      case 'pdo_mysql':
        try {
            if (!($dbhandle = new PDO('mysql:host='.$host.';dbname='.$db,$user,$pwd))) {
            if ($debug) echo '<div id="message">Reason: '.$dbhandle->errorInfo()[2].'</div>';
                return false;
            }
        } catch (Exception $e) {
            return false;
  ...
  switch ($CONFIG['database']['type']) {
    case 'mysql':
      if (!mysqli_query($dbhandle,$query)) {
        if ($debug) echo '<div id="message">Query failed: '.$query.'<br />Reason: '.mysqli_error($dhandle).'</div>';
          return false;
      }
      break;
    case 'pdo_mysql':
        if (!$dbhandle->query($query)) {
            if ($debug) echo '<div id="message">Query failed: '.$query.'<br />Reason: '.$dbhandle->errorInfo()[2].'</div>';
            return false;
        }
        break;
    case 'pgsql':
...
<br />&nbsp;<br />
<!-- footer -->
<div id="footer"><a href="http://openupload.sf.net">Open Upload</a> - Created by Alessandro Briosi &copy; 2019</div>
</div>
</body>
</html>
